...
from app.dependencies.auth import can_modify_content, get_current_user
from app.models import Resposta, Topico, Usuario
from app.schemas import RespostaCreate, RespostaResponse, RespostaUpdate

router = APIRouter(tags=["respostas"])


def _to_response(resposta: Resposta) -> RespostaResponse:
...
def list_respostas(topico_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    respostas = (
        ...
        .order_by(Resposta.created_at.asc())
        .all()
    )
    return [_to_response(r) for r in respostas]
